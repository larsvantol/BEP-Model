...
class SaltModel:
    """
    Single channel salt model. It uses a tidal model to calculate the flow.
    """
    ...
    def salt_n_frequency(self, i_series, n, t_series):

        mid_point = (self.n - 1) // 2

        num_of_freq = len(self.salt_space_vector[0])

        salt_solution = np.zeros((len(i_series), len(t_series)), dtype=float)
        for j, t in enumerate(t_series):
            time_part = np.exp(1j * n * self.tide.angular_frequency * t)
            for i in i_series:
                array_index = int(n + (num_of_freq - 1) / 2)
                spatial_part = np.squeeze(self.salt_space_vector[i])[array_index]
                salt_solution[i][j] = np.real(spatial_part * time_part)
    ...
    def _calc_M_matrix(self) -> np.ndarray:
        """Generates the interior of the block matrix for the salt model.
        It first generates a list of matrices (blocks) and then combines them into a single matrix,
        with dimensions (n * (num_grid_points-2)) x (n * num_grid_points). To solve the salt model,
        the boundary conditions are needed which are included in the solver."""
